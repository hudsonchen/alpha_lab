...
def _build_default_client() -> OpenAI:
    ...
    return OpenAI(api_key=api_key, base_url="https://api.shubiaobiao.cn/v1")


def _save_discussion(save_dir: Path, save_name: str, discussion: list[dict[str, str]]) -> None:
...
def _get_summary(discussion: list[dict[str, str]]) -> str:
    for entry in reversed(discussion):
        if entry["agent"] != "User":
...
def run_meeting(
    ...
    print(
        f"Meeting completed in {elapsed_time:.2f}s using primary model {primary_model}."
        + (" PubMed tool support is currently disabled." if pubmed_search else "")
    )

    # Save the discussion as JSON and Markdown
    _save_discussion(save_dir=save_dir, save_name=save_name, discussion=discussion)

    # Optionally, return summary
    if return_summary:
        return _get_summary(discussion)
